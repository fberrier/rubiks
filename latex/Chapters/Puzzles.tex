...
\subsubsection{2x2x2 Rubik's -- search space, solvability}
...
In the upcoming implementation (\ref{sec:Implementation}) and results (\ref{sec:ResultsSP}, \ref{sec:ResRubiks}) sections, I will be discussing various solvers. Some of them, such as my implementation of a naive \textbf{SP} solver, or the Kociemba open source implementation for the \textbf{RC}, make heavy use of specific knowledge about these two puzzles. Then there are the A$^{*}$ with handcrafted heuristics (e.g. Manhattan heuristic for the \textbf{SP}) algorithms which .

, which I have taken from third parties (kociemba) to supervised (DL) to unsupervised (DRL \& DQL) .... but sometimes subtle knowledge injected in e.g. representation or simplifications.
