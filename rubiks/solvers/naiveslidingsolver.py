...
class NaiveSlidingSolver(Solver):
    ...
    def reduce(self,
        ...
        self.log_info('We need to solve ', puzzle)
        n = puzzle.n
        m = puzzle.m
        ...
        while n > max(2, max_n) or m > max(2, max_m):
            if n >= m:
                self.log_info('Let us sort out top row')
                moves = self.solve_top_row(sliding_puzzle)
                for move in moves:
                    move.tile = (move.tile[0] + offset_n,
                                 move.tile[1] + offset_m)
                offset_n += 1
                self.log_info('We now have ', sliding_puzzle, ' ... ')
                sliding_puzzle = SlidingPuzzle(tiles=sliding_puzzle.tiles[1:])
                self.log_info(' ... and are left solving ', sliding_puzzle)
            else:
                self.log_info('Let us sort out left col')
        ...
        moves = list()
        self.log_info('offset_n: ', offset_m)
        self.log_info('offset_m: ', offset_n)
        if max_m < 2 or max_n < 2:
            while not sliding_puzzle.in_order():
                ...
                self.log_info('rotation move: ', move)
                self.log_info('sliding_puzzle -> ', sliding_puzzle)
                move.tile = (move.tile[0] + offset_n,
                             move.tile[1] + offset_m)
                self.log_info('rotation move with offsets: ', move)
                moves.append(move)
            solution.path.extend(moves)
            solution.cost = len(solution.path)
        return solution
    ...
    def move_value_to_target(self,
        ...
        while target_pos != current_pos:
            ...
            self.log_info(new_moves)
            sliding_puzzle = sliding_puzzle.apply_moves(new_moves)
            moves.extend(new_moves)
            current_pos = sliding_puzzle.find_tile(value)
            self.log_info('We applied ', new_moves)
            self.log_info('And got to ', sliding_puzzle, 'empty: ', sliding_puzzle.empty)
        positions_to_avoid.append(target_pos)
        self.log_info('positions_to_avoid: ', positions_to_avoid)
        return sliding_puzzle

    def solve_top_row(self, sliding_puzzle):
        assert sliding_puzzle.m >= 3
        self.log_info('solve_top_row for ', sliding_puzzle)
        ...
        self.log_info('desired_top_row: ', desired_top_row)
        target_positions = [(sliding_puzzle.n - 1, 0),
                            *[(0, pos) for pos in range(sliding_puzzle.m - 2)],
                            (0, sliding_puzzle.m - 1),
                            (1, sliding_puzzle.m - 1),
                            ]
        self.log_info('target_positions: ', target_positions)
        if sliding_puzzle.n == 3 and sliding_puzzle.m == 3:
            """ ugly special case """
            joker_value = desired_top_row[-1]
            self.log_info('special case dim 3 joker_value=', joker_value)
            desired_top_row = desired_top_row[1:-1]
            target_positions = target_positions[1:-1]
            self.log_info('special case dim 3 desired_top_row -> ', desired_top_row)
            self.log_info('special case dim 3 target_positions -> ', target_positions)
        ...
        for value, target_pos in zip(desired_top_row, target_positions):
            end_state = self.move_value_to_target(end_state,
                                                  ...
                                                  positions_to_avoid,
                                                  moves)
        self.log_info('end_state after loop: ', end_state)
        if sliding_puzzle.n == 3 and sliding_puzzle.m == 3:
            """ ugly special case """
            assert end_state.tiles[0][0] == desired_top_row[0]
            assert end_state.tiles[0][sliding_puzzle.m - 1] == sliding_puzzle.m - 1
            joker_pos = end_state.find_tile(joker_value)
            self.log_info('%d is in position ' % joker_value, joker_pos)
            empty = end_state.empty
            self.log_info('empty is in position ', empty)
            if joker_pos == (1, 1) and empty == (0, 1):
                move = Slide(1, 1)
                end_state = end_state.apply(move)
                moves.append(move)
                self.log_info('after moving empty to go to the only annoying solution: ', end_state)
                three = end_state.find_tile(3)
                assert three == (0, 1), 'WTF?'
            ...
            if joker_pos != (0, 1):
                end_state = self.move_value_to_target(end_state,
                                                      ...
                                                      positions_to_avoid,
                                                      moves)
                self.log_info('After special move we are in normal case', end_state)
                ...
                moves.extend(new_moves)
                self.log_info('Almost there: ', end_state)
                ...
                self.log_info(sliding_puzzle)
                return moves
        target_empty_pos = (0, sliding_puzzle.m - 2)
        new_moves = list()
        self.log_info('target_empty_pos: ', target_empty_pos)
        while end_state.empty != target_empty_pos:
            self.log_info('end_state.empty: ', end_state.empty)
            self.log_info('target_empty_pos: ', target_empty_pos)
    ...
    def solve_left_col(self, sliding_puzzle):
        ...
        #self.log_info('solve_left_col for ', sliding_puzzle)
        ...
        self.log_info('desired_left_col: ', desired_left_col)
        self.log_info('target_positions: ', target_positions)
        positions_to_avoid = list()
