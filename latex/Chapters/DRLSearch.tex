...
\While{epoch < max\_epochs \&\& other\_convergence\_criteria(net)}
...
\\ \teal{\algorithmiccomment{generate \textit{training} puzzles if criteria met}}\black
        \If{puzzles\_generation\_criteria($puzzles$)}
            \State{$puzzles \gets generate\_random\_states(puzzle\_type, n\_puzzles)$}
        \EndIf
        \State{$V \gets dict()$}
...
\\ \teal{\algorithmiccomment{perform value iteration}}\black
		\State{$child\_cost \gets \Call{Cost}{transition} + target\_net(child)$}
...
\\ \teal{\algorithmiccomment{iterate through network training running feed-forward and backward-prop}}\black
	\State{$net \gets \Call{TrainNeuralNet}{V, loss\_function, net}$}
...
\\ \teal{\algorithmiccomment{ ... update cost \textbf{and best action} ...}}\black
	...
	\EndFor
\\ \teal{\algorithmiccomment{ ... same as \textbf{DRL} ...}}\black
\State{$net \gets \Call{TrainNeuralNet}{Q\_and\_V, loss\_function, net}$}
\EndWhile
\EndFunction
\end{algorithmic}
\end{algorithm}
