...
class BrokenAxes:
    ...
    def subax_call(self, method, args, kwargs):
        ...
        result = []
        brokenaxis = False
        if len(args) > 0 and isinstance(args[0], str) and args[0] == 'whatahorriblehack':
            assert len(self.axs) == args[1]
            brokenaxis = True
        for count, ax in enumerate(self.axs):
            if brokenaxis:
                assert len(self.axs) == args[1]
                this_args = tuple(args[j][count] for j in range(2, len(args)))
            else:
                this_args = args
            if ax.xaxis.get_scale() == 'log':
            ...
            result.append(getattr(ax, method)(*this_args, **kwargs))
